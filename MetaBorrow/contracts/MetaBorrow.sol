...
import "./interfaces/IMetaBorrow.sol";

contract MetaBorrow is IMetaBorrow, ERC721Holder, ERC1155Receiver, ERC1155Holder {
    ...
    struct Lending {
        IMetaBorrow.NFTStandard nftStandard;
        ...
        private
    {
        ...
        while (_cd.right != _cd.nfts.length) {
            if (
                ...
                (_cd.nftStandard[_cd.right] == IMetaBorrow.NFTStandard.E1155)
            ) {
                _cd.right++;
            } else {
                _handler(_cd);
    ...
    function lend(
        IMetaBorrow.NFTStandard[] memory _nftStandard,
    ...
    function safeTransfer(
        ...
        if (_cd.nftStandard[_cd.left] == IMetaBorrow.NFTStandard.E721) {
            IERC721(_cd.nfts[_cd.left]).transferFrom(
                _from,
                _to,
                _cd.tokenIds[_cd.left]
            );
        } else if (_cd.nftStandard[_cd.left] == IMetaBorrow.NFTStandard.E1155) {
    ...
    function handleLend(CallData memory _cd) private {
        for (uint256 i = _cd.left; i < _cd.right; i++) {
            ...
            bool nftIs721 = (_cd.nftStandard[i] == IMetaBorrow.NFTStandard.E721);
