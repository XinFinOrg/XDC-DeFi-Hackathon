...
contract GetoBureau {
    ...
    constructor(IUniswapV2Factory _factory, address _bank, address _geto, address _weth) public {
        factory = _factory;
        geto = _geto;
        bank = _bank;
        weth = _weth;
    ...
    function _toWETH(address token) internal returns (uint256) {
        ...
        if (token == weth) {
            uint amount = IERC20(token).balanceOf(address(this));
            _safeTransfer(token, factory.getPair(weth, geto), amount);
            return amount;
        }
        ...
        pair.swap(amount0Out, amount1Out, factory.getPair(weth, geto), new bytes(0));
        return amountOut;
    }

    function _toSUSHI(uint256 amountIn) internal {
        IUniswapV2Pair pair = IUniswapV2Pair(factory.getPair(weth, geto));
        (uint reserve0, uint reserve1,) = pair.getReserves();
        address token0 = pair.token0();
